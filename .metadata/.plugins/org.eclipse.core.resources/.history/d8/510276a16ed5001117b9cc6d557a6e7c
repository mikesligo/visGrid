...
public class HistogramEditPart extends ShapeNodeEditPart {
	...
	protected boolean addFixedChild(EditPart childEditPart) {
		...
		if (childEditPart instanceof visGrid.diagram.edit.parts.HistogramFilenameEditPart) {
			((visGrid.diagram.edit.parts.HistogramFilenameEditPart) childEditPart)
					.setLabel(getPrimaryShape()
							.getFigureHistogramFilenameFigure());
			return true;
		}
		return false;
	}
	...
	public class HistogramFigure extends RectangleFigure {
		...
		private void createContents() {
			...
			histogramFigureSVG1.setMinimumSize(new Dimension(getMapMode()
					.DPtoLP(60), getMapMode().DPtoLP(60)));

			innerSVG0.add(histogramFigureSVG1, BorderLayout.CENTER);

			BorderLayout layoutHistogramFigureSVG1 = new BorderLayout();
			histogramFigureSVG1.setLayoutManager(layoutHistogramFigureSVG1);
			...
			inner0.add(fFigureHistogramNameFigure);

			FlowLayout layoutFFigureHistogramNameFigure = new FlowLayout();
			layoutFFigureHistogramNameFigure.setStretchMinorAxis(false);
			layoutFFigureHistogramNameFigure
					.setMinorAlignment(FlowLayout.ALIGN_LEFTTOP);

			layoutFFigureHistogramNameFigure
					.setMajorAlignment(FlowLayout.ALIGN_LEFTTOP);
			layoutFFigureHistogramNameFigure.setMajorSpacing(5);
			layoutFFigureHistogramNameFigure.setMinorSpacing(5);
			layoutFFigureHistogramNameFigure.setHorizontal(true);

			fFigureHistogramNameFigure
					.setLayoutManager(layoutFFigureHistogramNameFigure);

			fFigureHistogramFilenameFigure = new WrappingLabel();
			fFigureHistogramFilenameFigure.setText("<...>");

			inner0.add(fFigureHistogramFilenameFigure);

			FlowLayout layoutFFigureHistogramFilenameFigure = new FlowLayout();
			layoutFFigureHistogramFilenameFigure.setStretchMinorAxis(false);
			layoutFFigureHistogramFilenameFigure
					.setMinorAlignment(FlowLayout.ALIGN_LEFTTOP);

			layoutFFigureHistogramFilenameFigure
					.setMajorAlignment(FlowLayout.ALIGN_LEFTTOP);
			layoutFFigureHistogramFilenameFigure.setMajorSpacing(5);
			layoutFFigureHistogramFilenameFigure.setMinorSpacing(5);
			layoutFFigureHistogramFilenameFigure.setHorizontal(true);

			fFigureHistogramFilenameFigure
					.setLayoutManager(layoutFFigureHistogramFilenameFigure);
